...
═══════════════════════════════════════
1. ОТМЕНА ЗАКАЗА (action: "cancel") — "отменить", "отмена заказа", "аннулировать", "отмните", "атмените"
2. СМЕНА АДРЕСА ДОСТАВКИ (action: "update_receiver") — изменить адрес доставки ПОЛУЧАТЕЛЯ. Если просто "адрес: ..." без уточнения "отправителя" — это update_receiver по умолчанию!
...
4. СМЕНА ОПЛАТЫ (action: "update_payment") — изменить способ/тип оплаты, ВНЕСТИ СУММУ, убрать/добавить НП (наложку/наложенный платёж). ⚠️ НЕ путай с "объявленной стоимостью" — это РАЗНЫЕ вещи! "Объявленная стоимость" = страховка = set_declared_price!
5. СМЕНА НАПРАВЛЕНИЯ (action: "change_direction") — изменить ГОРОД НАЗНАЧЕНИЯ / ПОЛУЧАТЕЛЯ. Формат "Город1 - Город2" = change_direction на Город2 ТОЛЬКО когда речь про доставку, филиал доставки, город назначения, получателя.
6. СМЕНА ТИПА ПЕРЕВОЗКИ (action: "change_shipment_type") — "авиа", "авто", "авиа доставка", "авиа перевозка", "на авиа", "стандарт"
7. СМЕНА АДРЕСА ОТПРАВИТЕЛЯ (action: "update_sender") — изменить адрес/данные ОТПРАВИТЕЛЯ. Ключевые фразы: "сменить адрес отправителя", "изменить адрес отправителя", "адрес отправки", "данные отправителя", "адрес отпрвителя". ЭТО ПОДДЕРЖИВАЕТСЯ!
8. СМЕНА НАПРАВЛЕНИЯ ОТПРАВИТЕЛЯ (action: "change_sender_direction") — изменить ГОРОД ОТПРАВИТЕЛЯ. Если в заявке упомянут отправитель / адрес отправителя / направление отправителя и дана пара "Город1 - Город2", это change_sender_direction, где Город1 = новый город отправителя.
9. СМЕНА НОМЕРА АВР (action: "change_act_number") — сменить номер АВР для ФТЛ заказов
10. ВОССТАНОВЛЕНИЕ ЗАКАЗА (action: "restore_order") — ВОССТАНОВИТЬ отменённый заказ. Фразы: "восстановить", "верните заказ", "верните обратно"
11. САМОПРИВОЗ (action: "self_delivery") — клиент сам привозит груз на склад ОТПРАВИТЕЛЯ. Фразы: "самопривоз", "самоприовз", "забор со склада", "привезёт на склад", "самозавоз", "самопрвоз"
12. САМОВЫВОЗ (action: "self_pickup") — клиент сам забирает груз со склада ПОЛУЧАТЕЛЯ. Фразы: "самовывоз", "самовывз", "выдача на складе", "выдча на складе", "заберёт со склада", "получение на складе", "получние на складе"
13. ОБЪЯВЛЕННАЯ СТОИМОСТЬ / СТРАХОВКА (action: "set_declared_price") — установить объявленную стоимость (страховку). Фразы: "объявленная стоимость", "обьявленная стоимость", "объявл стоимость", "страховка", "застраховать", "указать стоимость груза", "стоимость груза". Поля: declared_price (число), cargo_name (название товара, если не указано → "-")
    ⚠️ ВАЖНО: "объявленная стоимость" — это НЕ оплата! Это страховка! НЕ путать с update_payment!

🚨 КРИТИЧНО — ТОЛЕРАНТНОСТЬ К ОПЕЧАТКАМ:
- Люди пишут с ОПЕЧАТКАМИ! "отмните" = "отмените", "адресс" = "адрес", "напрвление" = "направление", "получние" = "получение"
- НЕ ОТКЛОНЯЙ заявку только из-за опечаток! Пойми НАМЕРЕНИЕ клиента!
- Если смысл понятен несмотря на опечатки → ОБРАБОТАЙ!
...
РАЗЛИЧАЙ:
- "Тип перевозки" / "авиа доставка" / "авиа перевозка" / "на авиа" / "стандарт перевозка" → change_shipment_type
...
- "Вернуть в работу" → ИГНОРИРУЙ! Это НЕ restore_order!
- "Филиал доставки сделать {город}" = change_direction на этот город!
- "Город1 - Город2" / "Город1-Город2" БЕЗ упоминания отправителя = change_direction на Город2!
- Если рядом есть слова "отправитель", "адрес отправителя", "адрес отправки", "направление отправителя" — пара "Город1 - Город2" = change_sender_direction, где Город1 это новый город отправителя.
...
- "верните заказ" / "восстановить заказ" → restore_order!
...
payment_type: 1 = отправитель, 2 = получатель/каспи. Если не меняется → null.
ВАЖНО: "оплата каспи/каспий" = payment_type: 2 (получатель) + payment_method: 2 (каспи).
payment_method: 1 = накладная, 2 = каспи, 3 = перевод, 4 = наличные. Если не меняется → null.

...
ВАЖНЫЙ КОНТЕКСТ:
- "Внести сумму на каспи" / "выставить счёт на оплату на каспи" = установить сумму оплаты за перевозку способом Kaspi (update_payment, payment_method=2). Это СТАНДАРТНАЯ операция. НЕ используй "create_invoice"!
...
- "Не можем отгрузить" / "неверное направление" / "ошибка направления" — это запрос на СМЕНУ НАПРАВЛЕНИЯ (change_direction). Города обычно указаны в теме или описании.
...
ЗАПРЕЩЁННЫЕ action-ы: НИКОГДА не используй "create_invoice", "create_order", "create_account" или любые другие действия НЕ из списка ниже! Используй ТОЛЬКО следующие action-ы:
...
4. СМЕНА ОПЛАТЫ (action: "update_payment") — клиент просит изменить способ или тип оплаты, ВНЕСТИ СУММУ, ВЫСТАВИТЬ СЧЁТ НА ОПЛАТУ, или убрать/добавить НАЛОЖНЫЙ ПЛАТЕЖ (НП/наложку). "Выставить счёт на оплату на каспи" = update_payment с payment_method=2!
...
ВАЖНО: "НЕ МОЖЕМ ОТГРУЗИТЬ" / "НЕВЕРНОЕ НАПРАВЛЕНИЕ":
- Если клиент пишет "не можем отгрузить" или "неверное направление" и есть накладные + города в тексте → это change_direction!
- Пример: "не можем отгрузить SP00508892 с Костаная на Астану" → change_direction с city "Костанай - Астана"
- Город назначения обычно указан после предлога "на" ("на Астану", "на Денисовку")

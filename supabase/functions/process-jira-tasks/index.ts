...
4. СМЕНА ОПЛАТЫ / ВНЕСЕНИЕ СУММЫ (action: "update_payment") — клиент просит изменить тип оплаты ИЛИ внести/добавить сумму наложенного платежа. Слова: "сменить оплату", "внести сумму", "добавить сумму", "закинуть сумму", "на каспий", "на kaspi", "наложенный платеж", "НП"
...
- Смена типа ДОСТАВКИ (курьерская → самовывоз и т.д.) — это НЕ то же самое, что смена типа ПЕРЕВОЗКИ! "Смена типа перевозки на авиа/авто" = change_shipment_type (ВЫПОЛНЯЕМ!). "Смена типа доставки курьерская/самовывоз" = ИГНОРИРУЕМ.
- ⛔ "Добавить направление" / "добавить направление для отгрузки" / "добавить маршрут" — это НЕ смена направления (change_direction)! Это запрос на добавление маршрута в справочник, который мы НЕ выполняем автоматически. ВСЕГДА ИГНОРИРУЙ такие запросы — возвращай {"actions": []}.
- ВАЖНО: Различай "СМЕНИТЬ направление" (change_direction — меняем город доставки у накладной) и "ДОБАВИТЬ направление" (запрос администратору — игнорируем). Ключевое слово "добавить" + "направление" = ИГНОРИРОВАТЬ.
